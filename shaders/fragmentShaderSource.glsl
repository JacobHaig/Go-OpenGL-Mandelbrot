...
int iterateMandelbrot2(float c_re, float c_im, int max_iter){
    float z_re = 0.0;
    float z_im = 0.0;
...
void main() {
    ...
    float x = normalize( gl_FragCoord.x, 0, ress.x, -2.0,  1.0);
    float y = normalize( gl_FragCoord.y, 0, ress.y,  1.0, -1.0);
